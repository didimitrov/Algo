...
namespace _01.Tree
{
    internal class Tree
    {
        ...
        {
            var treeLeafNodes = Nodes.Where(n => n.Childrens.Count == 0).Select(n => n.Value).ToList();
        ...
        {
            var treeLeafNodes = Nodes.Where(n => n.HasParent && n.Childrens.Count != 0).Select(n => n.Value).ToList();
        ...
        private static void FindAllPathsWithGivenSum(int givenSum, List<int> curPath, List<List<int>> paths,
            TreeNode<int> root)
        {
            ...
            if (curSum > givenSum || root.Childrens.Count == 0)
            {
                return;
            }
